...
import os
import time
from PolGan import PanGan
from DataSet import DataSet
from config import FLAGES
#%%
def print_current_training_stats(error_pan_model, error_g_model, global_step, learning_rate, time_elapsed):
...
# def print_current_training_stats_valid(error_spatial,  global_step, learning_rate, time_elapsed):
...
#     print(losses + '\n')
#%%
model = PanGan(FLAGES.pan_size, FLAGES.ms_size, FLAGES.batch_size, FLAGES.ratio,
...
# dataset_valid = DataSet(FLAGES.pan_size, FLAGES.ms_size, FLAGES.img_path, FLAGES.data_path, FLAGES.batch_size, FLAGES.stride, 'valid')
...
with tf.Session() as sess:
    ...
    if FLAGES.is_pretrained:
        saver.restore(sess, './model_generator/PanNet-38000')
    for training_itr in range(FLAGES.iters):
        t1 = time.time()
        pan_batch, ms_batch, height_batch = next(DataGenerator)
        ...
        _, error_g_model, global_step, summary, learning_rate = sess.run(
            ...
            feed_dict={model.pan_img: pan_batch, model.ms_img: ms_batch, model.height_img: height_batch})
        error_pan_model = sess.run(model.spatial_loss, feed_dict={model.pan_img: pan_batch, model.ms_img: ms_batch,
                                                                  model.height_img: height_batch})
        print_current_training_stats(error_pan_model, error_g_model, global_step, learning_rate,
                                     time.time() - t1)
        ...
        if (global_step + 1) % FLAGES.model_save_iters == 0:
            ...
            saver_g.save(sess=sess, save_path=FLAGES.model_save_dir + '/' + 'Generator',
